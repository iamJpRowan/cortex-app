---
name: work-story
description: Orchestrate end-to-end story implementation. Validates readiness, creates a worktree and branch, creates a devlog, decomposes the story into inline tasks, waits for user approval, then drives each task to completion via /work-task subagents and opens a PR.
...
```bash
git worktree add .claude/worktrees/<story-slug> -b claude/<story-slug>
```

All subsequent file operations happen inside `.claude/worktrees/<story-slug>/`. Adjust all paths accordingly.

If a worktree or branch already exists for this slug, check whether work is in progress and surface that to the user before overwriting.
...
### Step 8: Commit setup (after approval)

Once the user approves, commit the story file and devlog changes before any implementation:
...
Also set the story's frontmatter `status` to `in progress` if it is not already.
