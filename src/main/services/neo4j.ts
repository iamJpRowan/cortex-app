  ...
  try {
    ...
    if (versionMatch) {
      ...
      if (majorVersion < 17) {
        return {
          available: false,
          error: `Java ${majorVersion} is not supported. Neo4j 5.x requires Java 17 or 21.\n` +
      ...
      if (majorVersion > 21) {
        console.warn(`[Neo4j] Java ${majorVersion} is not officially supported. Neo4j recommends Java 17 or 21.`)
    ...
    return {
      available: false,
      error: `Java Runtime Environment (JRE) 17 or 21 is required but not found.\n` +
             `Please install Java:\n  ${installInstructions}`
    }
  }
}
...
export async function startNeo4j(): Promise<Driver> {
  ...
  if (!fs.existsSync(binPath)) {
    ...
    throw new Error(
      `Neo4j binary not found at ${binPath}.\n${setupInstructions}`
    )
  }
  ...
  try {
    execSync(
      `"${adminBinPath}" dbms set-initial-password ${PASSWORD}`,
      {
        env: {
          ...process.env,
  ...
  if (neo4jProcess.killed || (neo4jProcess.exitCode !== null && neo4jProcess.exitCode !== 0)) {
    throw new Error(`Neo4j process exited with code ${neo4jProcess.exitCode}. Check the error logs above.`)
  }
...
async function waitForNeo4jReady(isReady: () => boolean, maxWaitSeconds = 60): Promise<void> {
