...
Derive the squash commit subject from the story's `type` and `title` fields using Conventional Commits format:

```
<type>(<scope>): <imperative summary of what was built, drawn from the story Goal>
```

Then merge:

```bash
gh pr merge --squash --delete-branch --subject "<conventional commit subject>"
```
