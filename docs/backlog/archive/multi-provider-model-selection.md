---
status: completed
completed_date: 2025-02-09
---

[Docs](../../README.md) / [Backlog](../README.md) / Archive / Multi-Provider Model Selection
...
1. Add **AI Elements model-selector** via existing pattern ([AI Elements](../../development/ai-elements.md)): `npx shadcn@latest add https://elements.ai-sdk.dev/api/registry/model-selector.json --path src/renderer/src/components/ai-elements -y`. Adapt to app data: `llm:listModels` → `ListModelsResult` / `ModelMetadata`; grouped by provider; handle unconfigured providers (disabled or hidden).
