...
related_files:
  ...
  - docs/backlog/bounded-tool-results-and-chat-ui-stability.md
...
- `ToolDefinitionMetadata.capResultLength` (optional, default `true`). When `capResultLength === false`, the factory skips the central cap. Documented in definition-types and in [adding-a-tool.md](../development/adding-a-tool.md) with the risk (freezes, "prompt too long").
